...
contains 
       REAL(4)FUNCTION O2ABS(TEMP,PRES,VAPDEN,FREQ)
!     name- o2abs_19.f  language- Fortran 77
...
!     ARGUMENTS:
       REAL(4)TEMP,PRES,VAPDEN,FREQ

      !INTEGER MIXING

!     NAME    UNITS    DESCRIPTION        VALID RANGE

!     TEMP    KELVIN   TEMPERATURE        245 to 334 (lab range)
...
!     D.S. Makarov et al, JQSRT v.112, pp.1420-1428 (2011). 
!     M.A. Koshelev et al, JQSRT, v.154, pp.24-27 (2015).
!     M.A. Koshelev et al, JQSRT, v.169, pp.91-95 (2016).
!     M.A. Koshelev et al, JQSRT, v.196, pp.78-86 (2017).
!     D.S. Makarov et al, JQSRT v.243 (March 2020) doi:10.1016/j.jqsrt.2019.106798
...
!        234-GHz line width from Drouin.
...
!     Local variables:
      INTEGER K,NL
      PARAMETER (NL=49)
      REAL(4)TH,TH1,B,PRESWV,PRESDA,DEN,DFNR,SUM,STR,Y,SF1,SF2
      REAL(4)DEL1,DEL2,GFAC,PE2,D1,D2,DF,DNU
      REAL(4)X,WB300,W300(NL),F(NL),Y0(NL),S300(NL),Y1(NL),BE(NL), &
...
! 1st-order mixing coeff. in 1/bar
